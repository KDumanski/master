...
export const SITES = [
  ...
  },
  {
    ...
    // dir switched to Omar-app 2026-07-13: the site was rebuilt as a static
    // Next.js export after the Hostinger VPS (old CMS host + the domain's DNS)
    // died. omarchaconart.com is dark until its DNS moves off the dead VPS
    // nameservers; the Pages URL is the live site meanwhile.
    owner: GH, repo: 'Omar-art-website', dir: 'Omar-app',
    stack: 'next', status: 'live', hosting: 'pages',
    domain: 'omarchaconart.com',
    url: pagesUrl(GH, 'Omar-art-website'),
    category: 'Client',
  ...
  },
  {
    ...
    // marboroughhousenh.com has NO NS delegation at the .com registry — the
    // registration lapsed/suspended at OnlyDomains (not a VPS casualty). The dead
    // domain was unhooked from Pages on 2026-07-14 so the site is reachable again
    // at its github.io URL; re-attach the custom domain once Keith renews it.
    owner: GH, repo: 'Marborough-House', dir: 'Marborough-House',
    stack: 'static', status: 'staging', hosting: 'pages',
    domain: null,
    url: pagesUrl(GH, 'Marborough-House'),
    category: 'Client',
    blurb: 'New Hampshire inn / hospitality site (domain expired — awaiting renewal).',
    manage: true,
  },
  {
    key: 'dental-group',
  ...
  {
    key: 'queens-perennial',
    name: 'Queens Perennial',
    owner: GH, repo: 'queens-perennial', dir: 'Queens Perennial',
    stack: 'static', status: 'staging', hosting: 'pages',
    domain: null,
    url: pagesUrl(GH, 'queens-perennial'),
    category: 'Client',
    blurb: 'Neighborhood flower CSA — fresh, seasonal, locally grown bouquets across NYC.',
    manage: true,
  },
  ...
  // ---------------- Sites staged on GitHub Pages (project pages) ----------------
  {
    key: 'fabians-tours',
    name: "Fabian's Tours",
    // Static Pages site since the VPS loss. atlantis.thehappyendings.org still has an
    // A record to the VPS (DNS at OnlyDomains, which we can't edit), so that vhost now
    // 301-redirects to the Pages URL below — the canonical home. Permanent fix: change
    // the OnlyDomains `atlantis` record to a CNAME -> kdumanski.github.io, then set the
    // custom domain on the repo.
    owner: GH, repo: 'fabians-tours', dir: 'Fabians Tours',
    stack: 'next', status: 'live', hosting: 'pages',
    domain: null,
    url: pagesUrl(GH, 'fabians-tours'),
    category: 'Client',
    blurb: 'Transformational Egypt water retreats (Oceanic Ventures).',
    manage: true,
  },
  // Retired 2026-07-14 (Keith): world-cup and Symbiotiks were unlisted here and their
  // GitHub Pages sites disabled. The repos (KDumanski/world-cup, KDumanski/Symbiotiks)
  // are untouched — re-add an entry here and re-enable Pages to bring either back.
  {
  ...
  },
  {
    key: 'revolver',
    name: 'The Sweep',
    owner: GH, repo: 'Revolver', dir: 'Revolver',
    // Front end for the Revolver news pipeline. Plain HTML/CSS/JS reading a
    // generated data/feed.json, so the static template deploys it as-is.
    // status stays 'staging' until the GitHub repo is created and pushed.
    stack: 'static', status: 'staging', hosting: 'pages',
    domain: null,
    url: pagesUrl(GH, 'Revolver'),
    category: 'Project',
    blurb: 'Fast, searchable news aggregator fed by the 30-minute sweep.',
    manage: true,
  },
];
